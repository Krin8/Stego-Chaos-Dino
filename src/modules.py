...
class MonaiFeaturizer(nn.Module):
    """MONAI ViT backbone for medical image feature extraction.

    Drop-in replacement for DinoFeaturizer with the same interface.
    Uses MONAI's 2D ViT architecture designed for medical imaging.

    Unlike DinoFeaturizer (frozen ImageNet-pretrained backbone), this
    backbone is trainable with random initialization. STEGO's contrastive
    loss will train it to learn medical-image-specific features.
    """
    ...
    def __init__(self, dim, cfg):
        ...
        self.cfg = cfg
        self.dim = dim
        patch_size = cfg.dino_patch_size   # reuse existing config key
        self.patch_size = patch_size

        # Match DINO ViT-Small architecture dimensions
        hidden_size = 384
        self.n_feats = hidden_size

        self.model = ViT(
            in_channels=3,
            img_size=(cfg.res, cfg.res),
            patch_size=(patch_size, patch_size),
            hidden_size=hidden_size,
            mlp_dim=hidden_size * 4,     # 1536
            num_layers=12,
            num_heads=6,
            spatial_dims=2,
        ...
        # Projection heads — identical to DinoFeaturizer
        self.cluster1 = self.make_clusterer(self.n_feats)
        self.proj_type = cfg.projection_type
        if self.proj_type == "nonlinear":
            self.cluster2 = self.make_nonlinear_clusterer(self.n_feats)

        print(f"[MonaiFeaturizer] Initialized with patch_size={patch_size}, "
              f"hidden_size={hidden_size}, img_size={cfg.res}, "
    ...
    def forward(self, img, n=1, return_class_feat=False):
        assert (img.shape[2] % self.patch_size == 0), \
            f"Image height {img.shape[2]} must be divisible by patch_size {self.patch_size}"
        assert (img.shape[3] % self.patch_size == 0), \
            f"Image width {img.shape[3]} must be divisible by patch_size {self.patch_size}"

        # MONAI ViT returns (patch_tokens, hidden_states_list)
        # patch_tokens: [B, num_patches, hidden_size]
        patch_tokens, _hidden_states = self.model(img)

        feat_h = img.shape[2] // self.patch_size
        feat_w = img.shape[3] // self.patch_size

        # Reshape to spatial feature map: [B, hidden_size, feat_h, feat_w]
        image_feat = patch_tokens.reshape(
            patch_tokens.shape[0], feat_h, feat_w, -1
        ).permute(0, 3, 1, 2)

        if return_class_feat:
            # Average pool patch tokens as a substitute for CLS token
            return image_feat.mean(dim=[2, 3], keepdim=True)

        # Project features (same logic as DinoFeaturizer)
        if self.proj_type is not None:
            code = self.cluster1(self.dropout(image_feat))
            if self.proj_type == "nonlinear":
                code += self.cluster2(self.dropout(image_feat))
